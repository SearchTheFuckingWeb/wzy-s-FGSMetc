...
FILE = 'ILSVRC2012_val_00000537.png'
...
def fgsm(model, X:Tensor, Y:Tensor, eps:float=8/255, alpha=1/255, steps:int=20) -> Tensor:
...
def pgd(model, X:Tensor, Y:Tensor, eps:float=8/255, alpha=1/255, steps:int=20) -> Tensor:
...
def uap(model, X:Tensor, Y:Tensor, eps:float=8/255, alpha=1/255, steps:int=20) -> Tensor:
  X_orig = X.detach().clone()
  AX = X_orig.detach().clone() + (torch.rand_like(X_orig) * 2 - 1) * eps
  for i in range(steps):
    AX.requires_grad = True
    ...
    AX = (X_orig + DX).clamp(0, 1).detach()
  return AX
#def mifgsm(model, X:Tensor, Y:Tensor, eps:float=8/255, alpha=1/255, steps:int=20) -> Tensor:
